...
import 'package:clipshare/core/app_state/app_state_provider.dart';
...
class _ClipShareAppState extends ConsumerState<ClipShareApp> {
  ...
  void initState() {
    ...
    // 回到前台时校验短信同步能力；放根组件可规避规则页在手机小屏下不常驻导致监听失效。
    _lifecycleListener = AppLifecycleListener(
      onResume: () {
        final appStateNotifier = ref.read(appStateProvider);
        if (appStateNotifier.initStatus == .initialized) {
          final rulesExecutor = ref.read(rulesExecutorProvider.notifier);
          rulesExecutor.ensureSmsSyncReady(showDialog: true);
        }
      },
    );
  }
          ...
          return AppThemeSwitcher(
            ...
            onThemeChanged: () async {
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                ...
                androidChannel.setHistoryFloatThemeMode(ThemeMode.system);
              });
            },
          );
        },
