...
class DeviceDao extends DatabaseAccessor<AppDatabase> with _$DeviceDaoMixin {
  ...
  Future<Device?> getById(String guid) {
    return (select(devices)..where((tbl) => tbl.guid.equals(guid))).getSingleOrNull();
  }
  ...
  /// 重命名设备展示名称。
  Future<int?> rename(String guid, String name) {
    return (update(devices)..where((tbl) => tbl.guid.equals(guid))).write(
  ...
  /// 删除指定设备。
  Future<int?> remove(String guid) {
    return (delete(devices)..where((tbl) => tbl.guid.equals(guid))).go();
  }

  /// 删除指定用户的全部设备。
  Future<int?> removeAll() {
    return delete(devices).go();
  }

  /// 更新设备当前连接地址。
  Future<int?> updateDeviceAddress(String guid, String address) {
    return (update(devices)..where((tbl) => tbl.guid.equals(guid))).write(
  ...
  /// 更新设备内网连接地址。
  Future<int?> updateDeviceInternalAddress(String guid, String address) {
    return (update(devices)..where((tbl) => tbl.guid.equals(guid))).write(
