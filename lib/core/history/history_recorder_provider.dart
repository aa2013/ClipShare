...
import 'package:clipshare/core/settings/clipboard/clipboard_settings.dart';
...
class HistoryRecorderNotifier extends _$HistoryRecorderNotifier {
  ...
  RulesExecutorNotifier get _rulesExecutor => ref.read(rulesExecutorProvider.notifier);

  ClipboardSourceNotifier get _sourceNotifier => ref.read(clipboardSourceProvider.notifier);
  ...
  void add(RawHistoryEvent event) {
    _rawEvents.add(event);
  }

  void addDelta(HistoryDeltaEvent event) {
  ...
  Future<void> _process(RawHistoryEvent event) async {
    ...
    final (historyResult, ruleResult) = await _applyRules(history, event.source);
  ...
  ///应用规则
  Future<(History?, RuleApplyResult?)> _applyRules(History history, ClipboardSource? source) async {
    final type = HistoryContentType.parse(history.type);
    var content = history.content;
    var applyResult = await _rulesExecutor.apply(type, content, source);
  ...
  Future<History> _processSource(History history, ClipboardSource? source) async {
    ...
    if (_clipboardSettings.sourceRecord || type == HistoryContentType.notification) {
      if (source != null) {
        // 记录来源到来源库，并通知其他设备同步
        final clipboardSource = ref.read(clipboardSourceProvider.notifier);
        await clipboardSource.addOrUpdate(
          AppInfo(
  ...
  Future<HistoryDeltaEvent?> _addData(History history, RuleApplyResult? applyResult, bool shouldSync, [bool notify = true]) async {
    ...
    if (_clipboardSettings.sendBroadcastOnAdd) {
      ...
      androidChannel.sendHistoryChangedBroadcast(contentType, history.content, history.devId, deviceState.getName(history.devId));
    }
    ...
    if (!shouldSync) {
      ...
      final sourceService = await ref.read(clipboardSourceProvider.future);
    ...
    if (isAndroid && shouldSync && _clipboardSettings.sourceRecordViaDumpsys) {
      ...
      unawaited(
          clipboardManager.getLatestWriteClipboardSource().then((source) async {
            ...
            history = history.copyWith(source: Value(source.id));
            // add source icon
            await _sourceNotifier.addOrUpdate(
              AppInfo(
                id: _idGenerator.nextId(),
                appId: source.id,
                devId: _self.id,
    ...
    if (tags.isNotEmpty) {
      ...
      for (var tag in tags) {
        unawaited(tagNotifier.add(HistoryTag(
          id: _idGenerator.nextId(),
          hisId: history.id,
          tagName: tag,
        )));
