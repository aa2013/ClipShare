...
class RulesExecutorNotifier extends _$RulesExecutorNotifier {
  ...
  /// 确保短信同步能力就绪：未启用短信规则时停止监听，已授权时开启监听，否则按需引导授权。
  Future<bool> ensureSmsSyncReady({
    ...
    if (!enableSmsSync) {
      await ref.read(androidChannelProvider.notifier).stopSmsListen();
      return false;
    }
    if (await PermissionHelper.testAndroidReadSms()) {
      await ref.read(androidChannelProvider.notifier).startSmsListen();
      return true;
    }
  ...
  /// 请求 Android 短信读取权限；必要时先弹出引导对话框，授权成功后开启短信监听。
  ...
  Future<RuleState> build() async {
    ...
    unawaited(ensureSmsSyncReady(showDialog: true));
    _loadAllLuaUserFn();
    return RuleState(
      rules: _rules,
  ...
  Future<bool> saveScriptModule(ScriptModule module, ScriptModule newValue) async {
    ...
    //copyWith 生成新实例，先保留 UI 新建标记再回填，避免丢失 isNewData
    final isNewData = newValue.isNewData;
    newValue = newValue.copyWith(
      version: DateTime.now().yyyyMMddHHmmss,
    );
    newValue.isNewData = isNewData;
    try {
      final Future<int> saveFuture;
