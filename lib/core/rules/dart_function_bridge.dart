...
late final ProviderContainer _refContainer;
...
int _log(Pointer<lua_State> L) {
  try {
    ...
    if (isTest) {
      RulesExecutorNotifier._testOutputs.add(
...
int _notify(Pointer<lua_State> L) {
  try {
    ...
    final notifier = _refContainer.read(notifyProvider.notifier);
    notifier.notify(
      title: title,
      content: content,
...
int _androidSendHistoryChangedBroadcast(Pointer<lua_State> L) {
  try {
    ...
    final androidChannel = _refContainer.read(androidChannelProvider.notifier);
    final contentType = HistoryContentType.parse(type);
    if (contentType == HistoryContentType.unknown) {
      return 0;
    }
    androidChannel.sendHistoryChangedBroadcast(
      contentType,
      content,
      fromDevId,
      fromDevName,
