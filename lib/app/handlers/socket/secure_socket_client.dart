...
class SecureSocketClient {
  ...
  static Future<SecureSocketClient> _createFromSocket({
    ...
    var ssc = SecureSocketClient._private(
      EndPoint(
        ...
        isSender ? socket.remotePort : socket.port,
      ),
    );
    ...
    ssc._isSender = isSender;
    ssc._listen();
    await ssc._sendInitData();
    try{
      final client = await ssc._connectedCompleter.future.timeout(timeout);
      ssc._connected = true;
      return client;
    }catch(_){
      await ssc.close(true);
      rethrow;
    }
  }
  ...
  Future<void> _listen() async {
    ...
    try {
      ...
      if (_connected) {
        ...
        _connectedCompleter.completeError('closed by another');
      }
    }
  }
  ...
  Future<void> _resolveMessagePacket(Uint8List packet) async {
    ...
    if (_connected) {
      Log.debug(tag, "receive msg(connected), devName = ${msgData.send.name} type: ${msgData.key}");
  ...
  Future<void> _onMsgConnect(MessageData msgData) async {
    var device = await deviceDao.getById(msgData.send.guid, 0);
  ...
  Future<void> _exchange(Uint8List packet) async {
    ...
    //A(client) <------- B(server)
      ...
      throw Exception('seq param not found, data = $str');
    }
    if (_keyIsExchanged) {
  ...
  ///发送自身信息给对方
  Future<void> _sendSelfInfo() async {
    //发送本机信息给对方
  ...
  ///关闭连接
  Future<void> close([bool manual = false]) async {
    try {
      if (manual) {
        _isClosedByUser = true;
      }
