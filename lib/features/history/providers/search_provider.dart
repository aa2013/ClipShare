...
class SearchNotifier extends _$SearchNotifier {
  ...
  /// 加载动画最短展示时长，避免查询过快导致列表闪烁。
  static const minLoadingDuration = Duration(milliseconds: 300);
  ...
  /// 查询搜索首页。
  Future<void> search() => _query(silent: false);

  /// 执行查询并写入结果。
  ///
  /// [silent] 为 true 时（后台实时重查）不展示加载动画、不补足最短时长，
  /// 直接原地更新列表。
  Future<void> _query({required bool silent}) async {
    final filter = state.filter;
    if (!silent) {
      state = state.copyWith(searching: true, loading: true);
    }
    if (!silent) {
      await Future.delayed(minLoadingDuration);
    }
