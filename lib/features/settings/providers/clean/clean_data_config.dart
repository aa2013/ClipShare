...
import 'package:clipshare/core/extensions/string_extension.dart';
...
class CleanDataConfig {
  ...
  factory CleanDataConfig.fromJson(String json) {
    ...
    return CleanDataConfig(tags: (map['tags'] as List<dynamic>).cast<String>(), devIds: (map['devIds'] as List<dynamic>).cast<String>(), contentTypes: (map['contentTypes'] as List<dynamic>).cast<String>().map(HistoryContentType.parse).where((e) => e != HistoryContentType.unknown).toList(), saveTopData: map['saveTopData'].toString().toBool(), removeFiles: map['removeFiles'].toString().toBool(), autoClean: map['autoClean'].toString().toBool(), cron: map['cron'], autoCleanFreq: freq == CleanDataFreq.unknown ? CleanDataFreq.day : freq, lastCleanTime: DateTime.tryParse(map['lastCleanTime']?.toString() ?? ''), nextCleanTime: DateTime.tryParse(map['nextCleanTime']?.toString() ?? ''), saveDays: map['saveDays']?.toString().toInt());
  }
